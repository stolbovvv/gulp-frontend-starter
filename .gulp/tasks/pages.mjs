...
export const handlePages = () => {
	...
	return src(config.path.src.pages)
		.pipe(plumber())
		.pipe(newer(config.path.dest.pages))
		.pipe(replace(iconStr, '$1sprite.svg#$2'))
		.pipe(_if(config.mode.prod, replace(stylesStr, '$1.min$2')))
		.pipe(_if(config.mode.prod, replace(scriptsStr, '$1.min$2')))
